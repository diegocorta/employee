...
@RequestMapping("/v1/calendars/{calendar-id}/standard-workdays")
public class CalendarStandardWorkdayController {
	
	private final ICalendarStandardWorkdayService service;
	...
	public ResponseEntity<Collection<EntityModel<CalendarStandardWorkdayDto>>> getCalendarStandardWorkdays(
	...
	public ResponseEntity<CollectionModel<EntityModel<CalendarStandardWorkdayDto>>> createCalendarStandardWorkdays(
			@Parameter(description = "The identifier of the user") @PathVariable(name = "calendar-id") Long calendarId,
			@Valid @RequestBody List<CalendarStandardWorkdayDto> employeeContractDto) {
		
		Set<Long> calendarIds = employeeContractDto
				.stream()
				.map(CalendarStandardWorkdayDto::getCalendarId).collect(Collectors.toSet());
	...
	public ResponseEntity<Void> deleteCalendarStandardWorkdays(
			@Parameter(description = "the identifier of the user") @PathVariable(name = "calendar-id") Long calendarId,
			@Valid @RequestBody List<CalendarStandardWorkdayDto> employeeContractDto) {
